...
- `src/`: primary code (gym envs, training scripts, config, monitor). Key entries: `red_gym_env_v2.py`, `baseline_fast_v2.py`, `run_pretrained_interactive.py`, `config.py`, `monitor.py`.
...
- `outputs/`: mp4 recordings from `run_pretrained_interactive.py` via `VideoMonitor`.
...
- Train v2: `scripts/train.sh` or `uv run src/baseline_fast_v2.py`.
